...
const WatchTrailerBtn: React.FC<WatchTrailerBtnProps> = ({ trailerUrl, setTrailerUrl }) => {
  const handleClick = () => {
    if (!trailerUrl) {
  ...
  return (
    <div
      onClick={handleClick}
      className="group flex h-[68px] py-m px-md justify-center items-center gap-m bg-white/10 backdrop-blur-2xl cursor-pointer overflow-hidden transition-all duration-500"
    >
      <svg
